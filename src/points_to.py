...
pyDatalog.create_terms('PointsTo, New, Assign, Store, Load, HeapField, X, Y, Z, O1, O2, O3') # COMPLETAR
...
class PointsToAnalyzer:
    ...
    def infer_from_facts(self, new_facts, assign_facts, store_facts, load_facts) -> tuple[list, list]:
        # si no tengo facts agrego valores dummy (para evitar que datalog falle)
        if not assign_facts:
            assign_facts = [(None, None)]
        if not new_facts:
            new_facts = [(None, None)]
        if not load_facts:
            load_facts = [(None, None, None)]
        if not store_facts:
            store_facts = [(None, None, None)]
        ...
        heap_field = [(str(a), str(b), str(c)) for (a, b, c) in HeapField(X, F, Y).data if a and b and c]
    ...
    def display_text(points_to: set, heap_field: set) -> None:
        ...
        if not points_to:
            print("No hay informacion de points_to")

        if not heap_field:
            print("No hay informacion de heap_field")
    ...
    def display_matplotlib(self, points_to: set, heap_field: set) -> None:
        ...
        fig = plt.figure()
        pos = nx.spring_layout(G, seed=42, k=1)  # para que siempre se vea igual
